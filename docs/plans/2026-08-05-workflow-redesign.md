...
### 원인 ① brainstorming 실행이 보장되지 않음

brainstorming을 호출한 21개 세션 추적. 세 가지가 동시에 관측됨.

**(a) 호출 시점이 늦다 — 6/21 = 29%가 구현 시작 후 호출**

| 세션 | 호출 전 편집 | 호출 전 사용자 턴 | 호출 후 질문 |
|---|---|---|---|
| 02758166 | **161회** | 52 | 0 |
| 2e6c5911 | 70회 | 11 | 6 |
| de4f2b02 | 11회 | 12 | 0 |

161회 편집 후에 호출된 세션이 있음. "설계 승인 전 구현 금지" 하드 게이트가 열리는 시점에 이미 작업이 끝나감 — 게이트가 작동할 수 없는 순서.

**(b) 실행 편차가 크다 — 호출 후 질문 0~11회**
...
같은 9단계 체크리스트를 받고도 세션마다 다르게 실행됨. **분산 자체가 증상** — 강제된 절차라면 편차가 이렇게 크지 않다.

**(c) 산출물 42% 미작성** — 설계 문서 없음.

> 주의: 초기 측정에서 "큰 작업일수록 건너뛴다"는 결론을 냈으나 **오류**였다. 호출 후 첫 편집까지만 세어 창이 좁았음. 전체 구간으로 다시 세면 대형 세션(이후 편집 50+)의 질문 중앙값 8회, 소형 2회로 오히려 반대.
...
**네 번째: 그 correction을 담던 3계층 시스템 자체가 죽어 있었다** (2026-08-05 실측).

| 계층 | 마지막 기록 | |
|---|---|---|
| Layer 1 `.claude/rules/correction-*.md` | 7/29 (cube-backend) | 살아있음 |
| Layer 2 Register (auto memory) | 4/08 | 4개월 정지 |
| Layer 3 Log (auto memory) | **3/16** | 4.5개월 정지 |

Layer 2·3은 `correction-memory` 스킬이 채우게 돼 있었는데 스킬이 호출되지 않았다.
살아남은 Layer 1은 스킬이 아니라 **자동 로드되는 룰**(`correction-protocol.md`)이 만든 것이다.

**correction·룰은 실패가 증명된 수단이고, 그 실패를 기록하는 장치마저 선언이면 같이 죽는다.**

### 원인 ⑤ correction이 원인 대신 증상을 기록함

cube-backend `no-system-writes-to-operator-owned-columns`가 실례다.

| | 룰이 적은 것 | 실제 문제 |
|---|---|---|
| 내용 | 운영자 소유 컬럼에 시스템이 값을 씀 | **요청하지 않은 예외 처리를 만듦** |
| 층위 | 증상 | 원인 |

컬럼에 값을 쓴 건 결과다. 범위 밖 중량을 만났을 때 요청도 없던 예외 처리를 스스로 설계했고, 그 처리가 값을 쓸 자리로 운영자 컬럼을 골랐을 뿐이다. 본문에는 원인이 이미 적혀 있다 — "요구사항이 아니라 내 설계 선택이었다". 제목과 규칙 3개가 그 문장을 안 따라갔다.

결과적으로 이 룰은 다음번을 못 막는다. 같은 원인이 다른 형태(새 상태값, 별도 테이블, 로그 분기)로 나오면 그대로 통과한다.
사고 하나에 룰 하나씩 쌓이는데 근본 원인은 그대로 남는 구조.
...
3. **선언인가 강제인가** → 룰·스킬은 선언. 네 번 실패했음
4. **인기 ≠ 적합** → superpowers는 1위지만 이 레포에서 42% 실패
5. **증상인가 원인인가** → 사고의 마지막 표면만 금지하면 같은 원인이 다른 형태로 재발한다. 원인이 안 잡히면 적지 말고 물어라

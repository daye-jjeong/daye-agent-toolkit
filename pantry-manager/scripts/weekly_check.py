...
def main():
    # 전체 식재료 조회
    all_items = pantry_io.query_items()
    ...
    # 유통기한 체크
    expiry_result = pantry_io.check_expiring_items(days_ahead=7)

    if expiry_result["expiring"]:
        report_lines.append(f"⏰ **이번 주 유통기한 임박:** {len(expiry_result['expiring'])}개")
        for item in expiry_result["expiring"][:5]:

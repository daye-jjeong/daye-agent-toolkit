...
### 🚧 블로커 해소 현황
- `blocked_resolution`이 있으면 반드시 점검.
- `resolved=true`: 같은 레포에서 후속 세션이 있어 해소된 것. 실제로 블로커가 풀린 건지, 우회한 건지 구분.
- `resolved=false`: 주 내에 후속 작업 없음. 진짜 외부 의존인지, 우선순위에서 밀린 건지 짚어라.
- 미해소 블로커가 2건 이상이면 "블로커 관리 패턴" 문제를 짚어라.

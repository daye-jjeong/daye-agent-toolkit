...
- LLM이 판단 가능하면 자동 해소 → `resolve-task` 호출.
...
- 해소된 건: `resolve-followup`으로 resolution_note와 함께 resolved.
...
5. **어제 follow_up 해소 점검**: `yesterday_followups`가 있으면 반드시 확인. `resolved=true`는 해소된 것, `resolved=false`는 오늘도 안 한 것.
   - 미해소 건이 있으면 왜 못 했는지 짚어라. 우선순위에서 밀린 건가, 잊은 건가.
   - 2일 연속 미해소면 "이건 follow_up이 아니라 방치"로 에스컬레이션.
   예: "어제 cube-backend follow_up '테스트 추가'가 오늘도 미해소. 잊은 건가, 의도적으로 미룬 건가?"
...
### 🚧 블로커/Follow-up 해소 현황
- `followup_status` 데이터를 점검. followup_chains 테이블 기반.
- `status=resolved`: 코칭에서 내용 기반으로 해소 판정된 것. 실제로 풀린 건지, 우회한 건지 구분.
- `status=open`: 아직 미해소. 진짜 외부 의존인지, 우선순위에서 밀린 건지 짚어라.
- `days_open >= 3`: 에스컬레이션 대상. "이건 follow_up이 아니라 방치"로 판단.
- 미해소 건이 2건 이상이면 "후속 작업 관리 패턴" 문제를 짚어라.

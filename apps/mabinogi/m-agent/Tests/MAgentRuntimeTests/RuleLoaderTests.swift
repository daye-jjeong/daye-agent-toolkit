...
// 계약: apps/mabinogi/README.md — 앱 데이터는 ~/.mabi/m-agent/,
...
func supportDirectoryContract() throws {
    unsetenv("MABI_HOME")
    let byDefault = try #require(MAgentPaths.supportDirectory())
    #expect(byDefault.lastPathComponent == "m-agent")
    ...
    let overridden = try #require(MAgentPaths.supportDirectory())
    #expect(overridden.path == "/tmp/mabi-swift-test/m-agent")
}

...
- `EnterWorktree` 도구 사용. 브랜치명: `fix/xxx`, `feat/xxx`. Agent `isolation: "worktree"` 금지
...
4. worktree → 5. 구현(TDD, 태스크 경계 커밋) → 6. 검증(`verification.md`)
...
### Workflow 사용 조건
사용자가 명시 요청할 때만 쓴다(하네스가 그렇게 강제한다). 적합: 독립·대량·기계적, 광범위 감사, 대형 마이그레이션.
부적합: 설계·계획·리뷰(병렬 금지), 소규모(1-3파일), 디버깅 초기. ultracode 상시 안 켬.

...
2. `status: in_progress` 또는 `status: blocked` 태스크 수집
3. 각 태스크의 마지막 `progress_log` 항목 확인 (누가, 언제, 뭘 했는지)

```markdown
## 활성 태스크
| ID | 태스크 | 마지막 작업 | 플랫폼 |
|----|--------|------------|--------|
| t-ronik-001 | 캘리봇 기획 재정리 | 2/11 as-is 정리 완료 | claude-code |
| t-ming-001 | 로컬 YAML 프로젝트 관리 | 2/11 구조 이동 | claude-code |
```

마지막 작업이 **다른 플랫폼**이었으면 하이라이트:
> 💡 t-ronik-001은 **openclaw**에서 마지막으로 작업함. progress_log 확인 필요.

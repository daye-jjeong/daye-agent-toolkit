...
| 파일 | 용도 | 티어 |
|------|------|------|
| `scripts/lint_docs.py` | 시스템 .md 정합성 린터 (메인) | Tier 1 |
| `scripts/audit_policy.py` | 정책/건강 감사 — 깨진 링크, deprecated 용어, 오래된 문서 검출 | Tier 1 |

## 자동화

| 스케줄 | 스크립트 | 설명 |
|--------|---------|------|
| 수동/주간 리뷰 | `lint_docs.py` | 전체 정합성 검사 |
| 수동/주간 리뷰 | `audit_policy.py` | 정책 건강 감사 + Telegram 알림 |

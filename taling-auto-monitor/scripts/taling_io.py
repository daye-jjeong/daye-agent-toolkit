"""
taling_io.py -- Obsidian vault 기반 탈잉 챌린지 데이터 I/O 모듈.

~/mingming-vault/taling/ 아래에 Dataview-queryable markdown 파일을 읽고 쓴다.
stdlib만 사용.
"""
...
VAULT_DIR = Path(os.environ.get("TALING_VAULT", "~/mingming-vault")).expanduser()
TALING_DIR = VAULT_DIR / "taling"

CATEGORIES = {
    "checklists": TALING_DIR / "checklists",
}


# -- 쓰기 ------------------------------------------------
def _format_frontmatter(frontmatter):
    """frontmatter dict를 YAML 문자열 라인 리스트로 변환."""
    fm_lines = ["---"]
...
def write_entry(category, filename, frontmatter, body=""):
    ...
    fm_lines = _format_frontmatter(frontmatter)
    content = "\n".join(fm_lines)
    if body:
        content += f"\n\n{body}\n"
    else:
...
def find_entry(category, filename):
    """카테고리에서 특정 파일을 찾아서 (filepath, frontmatter, body)를 반환."""
    cat_dir = CATEGORIES[category]
    fpath = cat_dir / filename
    if not fpath.exists():
        return None

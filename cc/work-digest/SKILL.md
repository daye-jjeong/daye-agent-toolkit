...
**토픽 내용:**
- tag는 주된 활동 기준 (코드 수정했으면 코딩, 확인만 했으면 ops)
- completed이면 결과 명시 (머지, spec 완료, 실패 등)
- follow_up은 구체적 (모호한 "대응" 금지)
- "cron 수정"은 부족. "cron 중복 해결 — PROPOSAL 상태 추가, dedup guard 테스트 작성, 머지 완료"는 충분.
...
체크 항목:
1. **기능 단위 병합**: 같은 목표의 연속 세션이 쪼개져 있으면 → 대표 세션에 병합 토픽 저장, 나머지 세션 토픽 삭제
2. **불필요 토픽 제거**: 1-2분 단순 명령(/exit, /clear)이 독립 토픽이면 인접 토픽에 포함하거나 삭제
3. **태그-활동 일치**: 코드 수정했는데 "리뷰", 확인만 했는데 "코딩" 등
4. **요약 품질**: 명령어 나열이 아닌 뭘/왜/결과 수준
5. **eval 혼입**: eval 세션이 실제 작업 토픽에 섞이지 않았는지

FAIL 시 토픽을 직접 수정(`update-topics`)하고 Gate B-1부터 재실행.
